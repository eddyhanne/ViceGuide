...
- **Newsletter, selbst gehostet, mit Double-Opt-In (Juli 2026).** Anmelde-Box im Footer, Bestätigungs- und Abmeldelink per Mail, Admin-Versand im Redaktions-Panel. Neue Tabelle `newsletter_subscribers`. Versand über authentifiziertes SMTP (`info@viceguide.de`-Postfach angelegt), mit Fallback auf PHP `mail()`. Zu aktivieren, sobald die `smtp_*`-Config gesetzt und die DNS/DKIM-Übertragung durch ist. Details in Abschnitt 2 und 7.
...
6. **Newsletter-Zustellbarkeit: SMTP aktivieren, bevor real versendet wird.** Der Versand über authentifiziertes SMTP ist **implementiert** (`api/mail.php`, dependency-freier Mini-Client, siehe Abschnitt 7). Zu tun ist nur noch: (a) die DNS-/DKIM-Übertragung des `@viceguide.de`-Postfachs muss durch sein (Hostinger zeigte anfangs "Domain funktioniert noch nicht vollständig"), (b) die `smtp_*`-Schlüssel in `config.php` mit den Hostinger-Zugangsdaten füllen (Host `smtp.hostinger.com`, Port 465/`ssl`, User `info@viceguide.de`). Ohne SMTP fällt der Code auf PHP `mail()` zurück, das ohne SPF/DKIM/DMARC leicht im Spam landet. `mail_from` steht bereits auf `info@viceguide.de`.
...
- `api/mail.php`: kein Endpunkt, sondern der gemeinsame Versand-Helfer. `vg_send_mail($cfg, $to, $subject, $html, $extraHeaders)` gibt true/false zurück und wirft nie. Wählt automatisch: **authentifiziertes SMTP** (`vg_smtp_send()`, selbst gehaltener dependency-freier AUTH-LOGIN-Client, Modi `ssl`/`tls`/`none`), sobald `smtp_host` und `smtp_user` gesetzt sind, sonst Fallback auf PHP `mail()` (`vg_mail_native()`). Dazu `vg_site_url()`/`vg_mail_from()`/`vg_extract_addr()`. SMTP signiert über den Hostinger-Mailserver mit DKIM, das ist der Zustellbarkeits-Hebel gegenüber `mail()`.
- **`config.php`-Schlüssel für Mail** (Vorlage in `config.sample.php`): `notify_email` (Empfänger der Kommentar-Benachrichtigung, leer = aus), `mail_from` (Absender aller Mails, echte Domain-Adresse, steht auf `info@viceguide.de`), `site_url` (Basis-URL für Links in Mails, ohne Schrägstrich), sowie `smtp_host`/`smtp_port`/`smtp_user`/`smtp_pass`/`smtp_secure` für den SMTP-Versand (leer = `mail()`-Fallback).
