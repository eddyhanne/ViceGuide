...
10. **www/non-www/index.html vereinheitlichen (SEO).** `www.viceguide.de`, `viceguide.de` und `viceguide.de/index.html` sind aktuell alle drei separat mit Status 200 erreichbar (Duplicate-Content-Risiko). Eine 301-Weiterleitung in `.htaccess` auf die kanonische Form `https://viceguide.de/` würde das sauber loesen. Noch nicht umgesetzt, aus einer externen SEO-Analyse (ChatGPT).
11. **Artikeltitel kürzen (SEO).** `article.php` haengt an jeden Titel " - ViceGuide" an, bei ohnehin schon langen Titeln kommt man leicht über 60 Zeichen (Suchergebnis schneidet dann ab). Eher redaktionelle Arbeit beim Titel-Schreiben als ein Code-Fix.
12. **Echter Autor statt "ViceGuide Redaktion" (SEO/E-E-A-T).** Aktuell immer die generische Organisation als Autor hinterlegt. Bewusste Entscheidung des Betreibers nötig (Klarname? Redaktionsteam-Seite?), kein reiner Coding-Task.
