...
6. **Newsletter-Versand: live, mit zwei Postfächern.** SMTP-Versand ist implementiert und aktiv. Zwei Hostinger-Postfächer sind angelegt: `info@viceguide.de` (Kommentar-Benachrichtigung, transaktional) und `newsletter@viceguide.de` (Double-Opt-In-Bestätigung und Newsletter-Versand, ausdrücklich getrennt). Beide brauchen ihr Passwort in `config.php` (`smtp_pass` bzw. `newsletter_smtp_pass`), Host `smtp.hostinger.com`, Port 465/`ssl`. Getestet: Anmeldung und Bestätigungsmail kommen sauber im Posteingang an. Aufmachung erledigt: gebrandetes HTML-Template (`vg_newsletter_wrap`) plus ein Knopf im Panel (`buildNewsletterDraft()`), der aus den 5 neuesten Artikeln automatisch einen Entwurf mit Bild-Karten und Weiterlesen-Links baut, den der Admin vor dem Senden frei anpassen kann. Zustellbarkeit: DMARC-DNS-Eintrag beim Betreiber offen (verbessert Inbox-Quote der frischen Absender-Domain).
