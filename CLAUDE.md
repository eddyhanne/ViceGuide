...
- **Echtes Lazy-Loading fuer Kachel-/Listen-Hintergrundbilder ueber IntersectionObserver.** Datenbank-Kacheln/-Listenzeilen (Charaktere, Fahrzeuge, Wildtiere, ...) zeigen ihr Foto ueber ein CSS-Hintergrundbild (`fitStyle()`/`cardVisual()`), nicht ueber ein `<img>`-Tag (siehe "CSS-Hintergrundbilder" in den offenen Aufgaben, dort geht es um die Anzeige-Technik selbst). CSS-Hintergrundbilder kennen aber kein natives `loading="lazy"`, dadurch haben frueher beim Wechsel in eine Sektion mit vielen Eintraegen alle Fotos gleichzeitig angefangen zu laden, spuerbar als "alles laedt nach". Geloest ohne Aenderung an der Zuschnitt-Logik: `fitStyleBase()` setzt nur Position/Groesse/Farbe sofort (kostet kein Netzwerk), die eigentliche Bild-URL liegt zunaechst nur in einem `data-bg`-Attribut (Klasse `.lazy-bg`). `initLazyBg()` beobachtet alle `.lazy-bg`-Elemente per `IntersectionObserver` (200px Vorlauf) und setzt `background-image` erst, wenn eine Kachel tatsaechlich in die Naehe des sichtbaren Bereichs kommt, danach wird das Attribut entfernt. Muss nach jedem Neu-Rendern der Kacheln/Listen aufgerufen werden (`drawDB()` beide Ansichten, `globalSearch()`), analog zum bereits vorhandenen `revealCards()` fuer die Eintritts-Animation. Modal-Kopfbild (`modalHead()`) und Artikel-Titelbild bleiben bewusst eager (sofort sichtbar nach Nutzeraktion, lazy waere dort kontraproduktiv). **Wichtige Ergaenzung:** `drawDB()` baut die Kacheln/Listenzeilen bei jedem Sektionswechsel komplett neu auf (`innerHTML=...`), auch beim erneuten Besuch einer bereits gesehenen Sektion. Ohne Gegenmassnahme wuerde `initLazyBg()` dabei jedes Mal von vorne die 200px-Verzoegerung durchlaufen, obwohl der Browser das Bild laengst im Cache haelt, sichtbar als staendiges leichtes "Nachladen"/Aufpoppen beim Hin- und Herwechseln zwischen Sektionen (z. B. Fahrzeuge und Waffen). Geloest ueber ein Set `seenBgUrls`, das sich merkt, welche Bild-URLs in dieser Sitzung schon einmal tatsaechlich angezeigt wurden: fuer solche URLs setzt `initLazyBg()` `background-image` sofort und synchron, ganz ohne IntersectionObserver-Umweg, nur wirklich neue/ungesehene Bilder durchlaufen weiterhin die verzoegerte Beobachtung.
...
- **Wichtige begriffliche Trennung: "Speichern" (pro Bearbeitung) vs. "Fertigstellen" (Editiermodus verlassen).** Der Knopf im Bild/Text-Editor (`ieApply()`) heisst bewusst "Speichern" und nicht mehr "Uebernehmen", er speichert genau diese eine Bearbeitung sofort in die Datenbank. "Fertigstellen" (der schwebende Knopf bzw. der Admin-Panel-Knopf) speichert selbst gar nichts, er schaltet nur den Editiermodus aus. Grund fuer die Umbenennung: der Betreiber hatte "Fertigstellen" faelschlich als eine Art Sammel-Speichern fuer alle Aenderungen verstanden und dadurch vermutlich einzelne Bild/Text-Editor-Dialoge geschlossen, ohne vorher auf den (damals) "Uebernehmen" genannten Knopf zu klicken, die Bearbeitung ging dadurch verloren, wirkte aber wie ein Speicher-Bug. Bei kuenftigen Aenderungen an diesem Wortlaut darauf achten, dass die beiden Aktionen klar unterscheidbar bleiben.
...
10. Artikel-Titelbilder auf ein festes 16:9-Seitenverhaeltnis umstellen (aktuell 16:10 ueber `.art-hero{aspect-ratio:16/10}`). Hintergrund: die meisten im Netz gefundenen Bilder kommen bereits im 16:9-Format, aktuell muss die Redaktion beim Zuschneiden entweder reinzoomen (Bildverlust an den Raendern) oder nimmt schwarze Balken oben/unten in Kauf. Bewusst zurueckgestellt, vom Betreiber explizit als Nachgang-Punkt markiert, nicht Teil der aktuellen Editiermodus-Runde.
