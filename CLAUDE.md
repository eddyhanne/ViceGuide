...
6. **Newsletter-Versand: live, mit zwei Postfächern.** SMTP-Versand ist implementiert und aktiv. Zwei Hostinger-Postfächer sind angelegt: `info@viceguide.de` (Kommentar-Benachrichtigung, transaktional) und `newsletter@viceguide.de` (Double-Opt-In-Bestätigung und Newsletter-Versand, ausdrücklich getrennt). Beide brauchen ihr Passwort in `config.php` (`smtp_pass` bzw. `newsletter_smtp_pass`), Host `smtp.hostinger.com`, Port 465/`ssl`. Getestet: Anmeldung und Bestätigungsmail kommen sauber im Posteingang an. Offen bleibt nur, den Newsletter inhaltlich attraktiver zu machen (Template, Aufmachung).
