...
- **Newsletter bewusst selbst gehostet statt externer Dienst.** Entscheidung des Betreibers gegen einen ESP (Brevo/MailerLite) und für die eigene MySQL/PHP-Infrastruktur: volle Kontrolle, keine Kosten, keine Drittanbieter-Datenweitergabe. Der ehrlich benannte Preis dafür ist die Zustellbarkeit (PHP `mail()` über Shared Hosting landet ohne SPF/DKIM/DMARC leicht im Spam) und die volle DSGVO-Verantwortung. Double-Opt-In ist Pflicht und umgesetzt. Nächster Schritt zur Absicherung: Versand auf authentifiziertes SMTP über ein `@viceguide.de`-Postfach umstellen (Abschnitt 6, offene Aufgabe).
...
- **Newsletter, selbst gehostet, mit Double-Opt-In (Juli 2026).** Anmelde-Box im Footer, Bestätigungs- und Abmeldelink per Mail, Admin-Versand im Redaktions-Panel. Neue Tabelle `newsletter_subscribers`. Versand über PHP `mail()`, Umstellung auf SMTP steht als offene Aufgabe (Zustellbarkeit). Details in Abschnitt 2 und 7.
...
6. **Newsletter-Zustellbarkeit absichern, bevor real versendet wird.** Der Newsletter ist selbst gehostet und verschickt über PHP `mail()` (siehe Abschnitt 7). Ohne saubere SPF/DKIM/DMARC-Einträge für viceguide.de landen Mails leicht im Spam. Sobald der Betreiber ein `@viceguide.de`-Postfach bei Hostinger hat, den Versand auf authentifiziertes SMTP (PHPMailer mit den Postfach-Zugangsdaten) umstellen, dann ist DKIM-Signierung sicher. `mail_from` auf eine echte Domain-Adresse setzen, nicht Gmail.
...
- `api/mail.php`: kein Endpunkt, sondern der gemeinsame Versand-Helfer. `vg_send_mail($cfg, $to, $subject, $html, $extraHeaders)` über PHP `mail()`, dazu `vg_site_url()`/`vg_mail_from()`. Wirft nie, gibt true/false zurück. **Zustellbarkeit:** ohne SPF/DKIM/DMARC für die Domain landet der Versand leicht im Spam, Umstellung auf authentifiziertes SMTP ist eine offene Aufgabe (Abschnitt 6).
- **Neue `config.php`-Schlüssel** (Vorlage in `config.sample.php`): `notify_email` (Empfänger der Kommentar-Benachrichtigung, leer = aus), `mail_from` (Absender aller Mails, sollte eine echte Domain-Adresse sein), `site_url` (Basis-URL für Links in Mails, ohne Schrägstrich am Ende).
