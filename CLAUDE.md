...
7. Kanal-Aufbau über Instagram hinaus (YouTube, TikTok, X, Reddit, Discord): Status und Priorität in KANAELE.md. Handles außer Instagram sind Stand jetzt nicht gesichert.
   - **Community-Sektion (Juli 2026):** bewirbt jetzt auf der Seite Instagram (`@viceguide`) und Discord. Zwei Discord-Wege: „Discord beitreten" (Einladungslink) und „Discord öffnen" für Mitglieder (direkt zum Server, erscheint erst, sobald `COMMUNITY.discordServerId` gesetzt ist, Server-ID vom Betreiber nachzutragen). **Offen:** Reddit (Subreddit anlegen, dann `redditUrl` setzen) und Live-Chat (eingebettetes Discord-Widget über `discordServerId`), beide bewusst später besprechen.
