 ...
 *
 * GET ?id=<Creator-ID>            -> Avatar
 * GET ?id=<Creator-ID>&field=cover -> Cover-/Banner-Bild
 */
...
$id = (int)($_GET['id'] ?? 0);
$col = (($_GET['field'] ?? '') === 'cover') ? 'cover' : 'avatar';

$stmt = $pdo->prepare("SELECT $col AS img FROM creators WHERE id = ?");
$stmt->execute([$id]);
